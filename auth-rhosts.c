...
/*
 * $Id: auth-rhosts.c,v 1.12 1999/11/17 17:04:39 tri Exp $
 * $Log: auth-rhosts.c,v $
 * Revision 1.12  1999/11/17 17:04:39  tri
 * 	Fixed copyright notices.
 *
 * Revision 1.11  1999/02/21 19:51:54  ylo
  ...
  while (userfile_gets(buf, sizeof(buf), uf))
    {
      ...
      /* Check whether this host is permitted to be in .[rs]hosts. */
      {
        ...
        if (perm_denied)
          {
            log_msg("Use of %.100s denied for %.100s", filename, host);
            continue;
          }
      }
