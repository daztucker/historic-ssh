...
void process_message(SocketEntry *e)
{
  ...
  switch (type)
    ...
    case SSH_AGENTC_REMOVE_ALL_RSA_IDENTITIES:
      ...
      break;
    case 206: /* SSH_AGENT_FORWARDING_NOTICE */
      /* This is a forwarding notice from ssh2.  Ignore this. */
      break;
    default:
...
int main(int ac, char **av)
{
  ...
  snprintf(socket_dir_name, sizeof(socket_dir_name),
      ...
      if (!creation_failed)
        {
          snprintf(buf, sizeof(buf),
                   "%s=%s", SSH_AUTHSOCKET_ENV_NAME, socket_name);
          putenv(xstrdup(buf));
          snprintf(buf, sizeof(buf), "SSH_AGENT_PID=%d", pid);
          putenv(xstrdup(buf));
        }
      execvp(av[1], av + 1);
      perror(av[1]);
